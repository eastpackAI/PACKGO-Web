...
**2026-09-25（发布前焦点收口）**：`ConversationDrawer.tsx` 在抽屉滑入过渡结束（`transitionend`）后
才把焦点交给共享输入框，并保留超时兜底；抽屉处于 `visibility: hidden` 时浏览器会拒绝聚焦，
这会让“打开 Packy 后直接打字”失效。关闭时仍把焦点还给触发控件。
