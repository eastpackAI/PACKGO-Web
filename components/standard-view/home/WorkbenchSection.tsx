...
import { demoBoundary, pages, workbench } from "@/config/standardHome";
import { Section } from "./Section";
import { PackyOpenButton, askPacky, noteToPacky } from "./StandardPackyControls";
...
interface WorkbenchSectionProps {
  /** 版块锚点，默认 `workbench`。 */
  id?: string;
  /** 首页总览里显示「打开完整工作台页」入口；`/workspace` 页面自身传 `false`。 */
 ...
 * 标准视图 · 浏览器本地工作台演示
 *
 * 承接 `PACKGO-Web-2` 最新 `components/site/WorkbenchSection.tsx` 的本地功能：
 * 登记需求、项目进度与卡点、询价记录、本地图稿列表、问 Packy。
 *
 * 边界（必须持续成立）：
 * 1. 数据只写这台设备的 `localStorage`，不落库、不上传、不调用任何外部接口；
 * 2. 询价记录只是本地演示记录，**不是正式报价**；正式报价由 PACKGO 出具；
 * 3. Packy 入口只派发事件，由共享会话负责后续，不在这里复制第二套对话。
 */
export function WorkbenchSection({ id = "workbench", showWorkspaceLink = true }: WorkbenchSectionProps = {}) {
  ...
  return (
    <Section
      ...
      action={showWorkspaceLink ? { label: "打开完整工作台页", href: pages.workspace } : undefined}
    >
      {/* 边界先讲清楚，再让客户开始填 */}
      <aside className="wb-boundary-banner" aria-label="工作台演示边界">
        <p className="wb-boundary-banner__title">{demoBoundary.title}</p>
        <ul className="wb-boundary-banner__list">
          {demoBoundary.points.map((line) => (
            <li key={line}>{line}</li>
          ))}
        </ul>
      </aside>
